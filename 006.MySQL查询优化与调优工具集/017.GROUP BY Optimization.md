...
mysql> 
```
##### 无索引测试: SELECT c1, MIN(c2) FROM group_by_ab GROUP BY c1; <sup>通过输出可以发现，使用了临时表</sup>
```sql
  # 已删除索引aaa
  mysql> explain SELECT c1, MIN(c2) FROM group_by_ab GROUP BY c1;
  +----+-------------+-------------+------------+------+---------------+------+---------+------+-------+----------+-----------------+
  ...
  mysql> 
```
##### 无索引测试:SELECT c1, c2 FROM group_by_ab WHERE c1 < '1385833848548454355' GROUP BY c1, c2; <sup>通过输出可以发现，使用了临时表</sup>
```sql
     # 已删除索引aaa
     mysql> explain SELECT c1, c2 FROM group_by_ab WHERE c1 < '1385833848548454355' GROUP BY c1, c2;
     +----+-------------+-------------+------------+------+---------------+------+---------+------+-------+----------+------------------------------+
