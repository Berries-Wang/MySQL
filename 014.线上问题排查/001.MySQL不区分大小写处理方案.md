# MySQL 不区分大小写处理方案
## 总结
1. 如果需要在保证性能的前提下，保证大小写敏感，可以采用治标的方法(修改排序规则 'COLLATE' )。
