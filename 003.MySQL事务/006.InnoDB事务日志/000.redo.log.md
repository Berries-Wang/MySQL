...
---
## Checkpoint 技术 <sup>将重做日志增量刷新到磁盘上</sup>
&nbsp;&nbsp;页的操作首先在缓冲池中完成的，如果一条DML语句，如Update 或者 Delete改变了页中的记录，那么此时页是脏的，即缓冲池中的页的版本比磁盘的新，数据库需要将新版本的页从缓冲池刷新到磁盘。

&nbsp;&nbsp;当前事务数据库普遍都采用了WriteAheadLog策略，即当事务提交时，先写重做日志，再修改页。当由于发生宕机而导致数据丢失时，通过重做日志来完成数据的恢复，这也是事务持久性的要求。

&nbsp;&nbsp;checkpoint 主要解决的问题:
1. 缓冲池无法缓存数据库中所有的数据，当缓冲池不够用时，将脏页刷新到磁盘。
2. 重做日志无法无限增大，重做日志不可用时，刷新脏页
3. 缩短数据库宕机后的恢复时间，宕机重启时，数据库不需要重做所有日志，因为checkpoint之前的页都已经刷新回磁盘，故数据库只需对checkpoint之后的重做日志进行恢复。
